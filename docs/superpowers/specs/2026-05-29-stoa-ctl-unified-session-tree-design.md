...
>
> 本文件是本特性的唯一权威 spec。implementation plan、research audit 和 task breakdown 只能细化执行，不得弱化、覆盖或改写本文件中的 contract。
...
### A. 保留独立 meta session，只给普通 session 补桥接能力
...
- 继续保留两套产品概念和两套控制心智模型
- `stoa-ctl` 与前端 session tree 仍然不会天然对齐
- sub session 的前端同步仍然只能靠旁路补丁补回主视图

### B. 给所有 session 暴露 `stoa-ctl`，但保持全局平面可见

不推荐。

问题：

- session 内部会天然看见跨 branch / 跨 tree 对象，控制边界过宽
- 无法把“同级协作、纵向管控、横向不可破坏”表达成稳定 contract
- 前端即使做成树，也只是 UI 形态，控制面仍然是平面全局语义

### C. 统一 session tree + 统一 command surface + tree-local visibility

这是推荐方案，也是本 spec 采用的方案。

理由：

- 统一 session 管理面比并行的 meta-session 产品面更符合通用 agent session 管理形态
  见：[research/2026-05-29-unified-session-tree-external-practices-addendum.md](/D:/Data/DEV/ultra_simple_panel/.worktrees/unified-session-tree/research/2026-05-29-unified-session-tree-external-practices-addendum.md:17)
- 本地与后台创建的 session 都应同步进入同一共享 session 视图，而不是依赖手动刷新
  见：[research/2026-05-29-unified-session-tree-external-practices-addendum.md](/D:/Data/DEV/ultra_simple_panel/.worktrees/unified-session-tree/research/2026-05-29-unified-session-tree-external-practices-addendum.md:19)
- 前端正确性路径应是 bootstrap snapshot + main-to-renderer push，而不是 polling
  见：[research/2026-05-29-unified-session-tree-external-practices-addendum.md](/D:/Data/DEV/ultra_simple_panel/.worktrees/unified-session-tree/research/2026-05-29-unified-session-tree-external-practices-addendum.md:21), [research/2026-05-29-unified-session-tree-external-practices-addendum.md](/D:/Data/DEV/ultra_simple_panel/.worktrees/unified-session-tree/research/2026-05-29-unified-session-tree-external-practices-addendum.md:25)
- Stoa 的 same-depth-plus-descendants 可见性是明确的产品特化规则，应由单一 visibility service 集中实现，而不是依赖上游隐式行为
  见：[research/2026-05-29-unified-session-tree-external-practices-addendum.md](/D:/Data/DEV/ultra_simple_panel/.worktrees/unified-session-tree/research/2026-05-29-unified-session-tree-external-practices-addendum.md:27)

因此本 spec 明确收束为：

- 一个产品级 session 管理面
- 一个主进程权威 session graph
- 一个统一 `stoa-ctl` 命令面
- 一个 Stoa 自定义的 tree-local visibility / authority contract
...
### 6. Tree-local Peer Collaboration 是 Stoa 特化策略

社区里的更保守默认值通常是：

- `self-only`
- 或显式 handoff / explicit delegation

本设计明确不采用“全局全可见”，也不采用“只能看自己”。

Stoa 在 session 内部选择的协作策略是：

- 同 depth peer 可见并允许 `inspect` / `prompt`
- descendants 可见并允许更强控制
- ancestors 与其它 branch 继续不可见

这是 Stoa 的产品特化协作规则，不是通用框架的隐含默认值。
...
主机侧对 renderer、`stoa-ctl inspect` 和 `session:graph-event` 暴露的读模型为：
...
- `rootSessionId` / `depth` / `childCount` / `descendantCount` 只在 read model 中出现，不写回磁盘
- renderer 的 collapse / expand / active row 等 UI 状态属于 view state，不回写 session 模型
...
```text
V(S) = { X in T | depth(X) = depth(S) } U descendants(S)
```

其中 same-depth 集合天然包含 `S` 自己。
...
- 可见集合始终以调用者自己的 `rootSessionId` 与 `depth` 重新计算，不允许继承 parent 的可见结果
- same-depth peer 的 descendants 在任何 caller-filtered 输出里都不能作为“半可见 branch”泄漏出来
- archived 只是对象状态，不会扩大可见集合
- 任意 list / inspect / tree 输出都必须先做 visibility filter，再做 display projection
...
实时 graph 同步属于 renderer + IPC 主路径，不属于 session caller 的必备 CLI contract。
...
- 成功返回必须包含新节点的完整 `SessionNodeSnapshot`
...
- 任意成功的 graph mutation 都必须产出完整 `SessionNodeSnapshot`
- 任意成功的 graph mutation 都必须对应一条 `SessionGraphEvent`
- 可见性与 authority 的判断只能走它统一委托的 `SessionVisibilityService`
...
- create / inspect / prompt / destroy 语义必须与 renderer IPC 走同一 supervisor contract
...
### 0. Snapshot + Push 是唯一正确同步主路径

- renderer 启动时拉取一次完整 `SessionNodeSnapshot[]` bootstrap snapshot
- bootstrap 之后的所有 graph 增量变化只通过 `session:graph-event` push 进入 store
- `session:create` / `session:destroy` / `session:restore` invoke 返回值可以辅助本地 UX，但不能替代 push event 作为权威增量来源
- 不允许用 polling、定时 full reload、切 project 重进页面或手动 refresh 作为 correctness path
- 前端在同一运行期内必须看见后台新建的 child session，不允许要求 app restart 才可见

### 1. `session:graph-event` 成为统一 upsert 入口

`session:graph-event` 必须覆盖：
...
- 任意成功的 create / update / archive / restore / destroy，都必须沿 `main -> preload -> renderer` 的 `session:graph-event` 路径发出对应 `SessionGraphEvent`
- 旧平面 `session:event` / `SessionSummaryEvent` push contract 删除，不保留并行双轨事件源

### 2. renderer 收到未知 session 时必须插入

store 不能再假设事件只会更新已存在对象。

必须提供统一 `upsertSession(node)` 语义：
...
只要主机侧确认创建成功并广播 `session:graph-event`，前端都必须能看到。

这个规则额外包含：

- 不要求用户手动刷新
- 不要求用户切换 project / panel
- 不要求用户重启应用
...
### 4.1 Out-of-order event resilience

renderer store 必须先按 `session.id` 建立权威索引，再派生树结构。

这是为了覆盖：

- child 事件先到、parent snapshot 后到
- archived subtree root 先到、其它 descendants 后到
- restore 过程中 subtree 分批 upsert

约束：

- 任何未知 child 节点都不能被直接丢弃
- parent 尚未到达时，允许先作为 pending node 存在
- 一旦 parent 到达，必须自动恢复 parent / child 链接
- 不允许把“暂时缺 parent”的节点永久降级成平面孤立行

### 4.2 Archived projection 也保留 tree 结构
...
- `session:destroy`
- `session:inspect`
- `session:graph-event` 的 `SessionGraphEvent` push 订阅
...
- preload 必须向 renderer 暴露图事件订阅能力，而不是只暴露平面 `SessionSummaryEvent`
- 本期推荐的显式通道名就是 `session:graph-event`，不要继续复用旧平面事件名承载树图事件
...
- lineage / authority 失败时，`nextSteps` 必须给出可执行提示，而不是只给抽象报错
...
- 后台 child create 触发 `session:graph-event`
- `session:graph-event` create/update/archive/restore/destroy 都走 `SessionGraphEvent`
...
- 后台创建的 child session 显示不依赖刷新、切 panel 或重启应用
...
- [research/2026-05-29-stoa-ctl-current-architecture.md](</D:/Data/DEV/ultra_simple_panel/.worktrees/unified-session-tree/research/2026-05-29-stoa-ctl-current-architecture.md>)
- [research/2026-05-29-session-backend-topology.md](</D:/Data/DEV/ultra_simple_panel/.worktrees/unified-session-tree/research/2026-05-29-session-backend-topology.md>)
- [research/2026-05-29-session-frontend-topology.md](</D:/Data/DEV/ultra_simple_panel/.worktrees/unified-session-tree/research/2026-05-29-session-frontend-topology.md>)
- [research/2026-05-29-orca-cli-session-patterns.md](</D:/Data/DEV/ultra_simple_panel/.worktrees/unified-session-tree/research/2026-05-29-orca-cli-session-patterns.md>)
- [research/2026-05-29-session-cli-best-practices.md](</D:/Data/DEV/ultra_simple_panel/.worktrees/unified-session-tree/research/2026-05-29-session-cli-best-practices.md>)
- [research/2026-05-29-orca-frontend-child-visibility-subagent.md](</D:/Data/DEV/ultra_simple_panel/.worktrees/unified-session-tree/research/2026-05-29-orca-frontend-child-visibility-subagent.md>)
- [research/2026-05-29-orca-permission-visibility-subagent.md](</D:/Data/DEV/ultra_simple_panel/.worktrees/unified-session-tree/research/2026-05-29-orca-permission-visibility-subagent.md>)
- [research/2026-05-29-research-coverage-unified-session-tree-requirements.md](</D:/Data/DEV/ultra_simple_panel/.worktrees/unified-session-tree/research/2026-05-29-research-coverage-unified-session-tree-requirements.md>)
- [research/2026-05-29-spec-coverage-visibility-authority-frontend-sync.md](</D:/Data/DEV/ultra_simple_panel/.worktrees/unified-session-tree/research/2026-05-29-spec-coverage-visibility-authority-frontend-sync.md>)
- [research/2026-05-29-unified-session-tree-external-practices-addendum.md](</D:/Data/DEV/ultra_simple_panel/.worktrees/unified-session-tree/research/2026-05-29-unified-session-tree-external-practices-addendum.md>)
