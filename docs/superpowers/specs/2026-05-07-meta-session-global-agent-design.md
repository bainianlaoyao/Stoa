# Meta Session 全局控制面设计

日期：2026-05-07

> 术语说明：本设计最初以 `Hermes` 命名。当前仓库实现与命名已经统一为 `Meta Session` / `meta-session`。下文若仍保留少量 `Hermes` 历史草案表述，应按 `Meta Session` 语义理解；其中涉及 `hermes-agent` 的旧 provider 草案仅代表历史方案，不再代表当前实现 contract。
