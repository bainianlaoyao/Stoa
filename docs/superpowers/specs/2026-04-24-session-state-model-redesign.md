...
| `idle` | agent 没有未读完成结果，且可以接收下一步。 | 用户访问/激活 complete 会话后 |
| `working` | agent 正在处理当前轮。 | Claude `UserPromptSubmit` / `PreToolUse` |
| `complete` | agent 当前轮已完成，但用户尚未访问该完成结果。 | Claude `Stop`、OpenCode `session.idle`、Codex turn complete |
...
`complete` 是重要的长期状态，不是短暂事件。规则是：如果一轮从 `working` 结束，而该 session 没有被用户访问，则进入 `complete`。只有用户访问/激活该 session，确认看过完成结果后，才允许 `complete -> idle`。这样 UI 可以区分“已经完成但用户还没看”和“空闲 ready”。
...
`agent.completion_seen` 的来源不是 provider，而是 Stoa UI/主进程：当用户选择、激活、打开一个处于 `complete` 的 session，并且该 session 的 terminal/replay 已经可见时，Stoa 发送该 intent。它表示用户已经有机会看到完成结果，因此可以从 `Complete` 降为 `Ready`。
...
| `complete` | `agent.turn_started` | `working` | 用户在已完成会话上发起新 turn，视为已访问旧结果并进入新工作。 |
...
| `complete` | `agent.tool_started` | `working` | 新工作证据覆盖旧完成态。 |
| `working` | `agent.tool_started` | `working` | 保持 Running，更新摘要/工具名。 |
| `working` | `agent.turn_completed` | `complete` | Running -> Complete。完成但用户未访问。 |
| `complete` | `agent.completion_seen` | `idle` | Complete -> Ready。用户访问/激活后发生。 |
| `working` | `agent.permission_requested` | `blocked` | Running -> Blocked。 |
| `idle` | `agent.permission_requested` | `blocked` | Ready -> Blocked，少见但合法。 |
| `complete` | `agent.permission_requested` | `blocked` | 完成后又出现 provider 级阻塞，少见但按新证据处理。 |
...
- `blocked -> complete` 不能由 stale `agent.turn_completed` 触发。
- `error -> idle` 不能由 stale `agent.turn_completed` 触发。
- `complete -> idle` 只能由 `agent.completion_seen` 触发，不能由时间流逝、runtime alive 或任意 renderer hydrate 触发。
...
| `Running` | `agent.turn_completed` | `Complete` | Claude `Stop`，完成但未访问。 |
| `Complete` | 用户访问/激活该 session | `Ready` | `agent.completion_seen`。 |
...
- `Stop` -> `intent = agent.turn_completed`, `agentState = complete`
...
- `session.idle` -> `intent = agent.turn_completed`, `agentState = complete`
...
- turn complete notify -> `intent = agent.turn_completed`, `agentState = complete`
...
- `agent.turn_completed` 表示完成事件，最终稳定状态是 `agentState = complete`，UI 显示 `Complete`。
- `agent.completion_seen` 表示用户已访问完成结果，最终稳定状态是 `agentState = idle`，UI 显示 `Ready`。
...
- `complete + completion_seen` 进入 Ready。
