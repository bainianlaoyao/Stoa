 ...
 *
 * Extracted from `src/core/meta-session-context-assembler.ts` to stoa-server.
 * The constructor accepts injected dependencies for snapshot, presence, events,
 * and terminal replay — the same interfaces as the original.
 */
import type { BootstrapState, SessionSummary } from 'stoa-shared'
import type { SessionPresenceSnapshot } from 'stoa-shared'

// ---------------------------------------------------------------------------
// Text helpers — inlined from src/core/context/full-text-context.ts
// ---------------------------------------------------------------------------
...
function appendSection(lines: string[], label: string, value: string | null | undefined): void {
  if (!value) {
    return
  }
...
// ---------------------------------------------------------------------------
// Dependencies — injected interfaces matching the original codebase
// ---------------------------------------------------------------------------
...
export interface MetaSessionContextAssemblerDeps {
...
// ---------------------------------------------------------------------------
// Payload extraction — matches original readableTextFromPayload
// ---------------------------------------------------------------------------
...
export class MetaSessionContextAssembler {
  constructor(private readonly deps: MetaSessionContextAssemblerDeps) {}
  ...
  getEvents(
    ...
    return this.deps.listSessionEvents(sessionId, {
  ...
  getStatus(sessionId: string): { level: 'status'; session: SessionSummary; presence: SessionPresenceSnapshot | null } {
    ...
    return {
      level: 'status',
      session,
      presence: this.deps.getSessionPresence(sessionId)
    }
  }
  ...
  async getFullContext(
    ...
    appendSection(lines, 'Terminal', stripAnsi(await this.deps.getTerminalReplay(sessionId)))
  ...
  private requireSession(sessionId: string): SessionSummary {
    const session = this.deps.snapshotSource.snapshot().sessions.find((candidate) => candidate.id === sessionId)
    if (!session) {
