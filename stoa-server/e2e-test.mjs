...
const TEST_STOA_DIR = join(tmpdir(), `stoa-server-e2e-${process.pid}`)
const AUTH_TOKEN = 'stoa-e2e-token'

async function main() {
  await removeDirWithRetry(TEST_STOA_DIR)
  mkdirSync(TEST_STOA_DIR, { recursive: true })
  ...
  try {
    ...
    // 5. Create Session without runtime provider
    console.log('\n--- 5. Create Session without runtime provider ---')
    ...
    body = await res.json()
    check('create-session without runtime → 503', 503, res.status)
    check('create-session without runtime not ok', false, body.ok)
    check('create-session reports runtime bridge missing', 'runtime_unavailable', body.error?.code)
    ...
    check('bootstrap has no leaked failed session', 0, body.data.sessions.length)
    ...
    check('list-sessions count', 0, body.data.length)
    ...
    // 15. Final bootstrap (empty again)
    console.log('\n--- 15. Final Bootstrap ---')
    res = await fetch(`${BASE}/api/v1/bootstrap`, { headers: AUTH })
    body = await res.json()
