...
describe('ProjectSessionManager', () => {
  ...
  test('serializes concurrent persist calls so disk writes never overlap', async () => {
    vi.resetModules()
    const alphaPath = await createTempProjectDir('persist-alpha')
    const betaPath = await createTempProjectDir('persist-beta')
    ...
    const project1 = await manager.createProject({ name: 'alpha', path: alphaPath })
    const project2 = await manager.createProject({ name: 'beta', path: betaPath })
  ...
  test('createSession generates provider title when title is omitted', async () => {
    ...
    test('createSession initializes subagent facade fields for child sessions', async () => {
      const globalStatePath = await createTempGlobalStatePath()
      const projectDir = await createTempProjectDir()
      const manager = await ProjectSessionManager.create({ webhookPort: null, globalStatePath })
      const project = await manager.createProject({ path: projectDir, name: 'tree' })
    ...
    test('updateSubagentFacade persists facade fields and reloads them', async () => {
      const globalStatePath = await createTempGlobalStatePath()
      const projectDir = await createTempProjectDir()
      const manager = await ProjectSessionManager.create({ webhookPort: null, globalStatePath })
      const project = await manager.createProject({ path: projectDir, name: 'test' })
      const parent = await manager.createSession({ projectId: project.id, type: 'claude-code', title: 'parent' })
      const child = await manager.createSession({
        projectId: project.id,
        type: 'codex',
        title: 'child',
      ...
      const updated = await manager.updateSubagentFacade(child.id, {
        subagentName: 'ryu',
        subagentInputEpoch: 2,
        subagentLatestInputAt: '2026-06-11T00:00:00.000Z',
      ...
      expect(persisted.sessions.find((session) => session.session_id === child.id)).toMatchObject({
        ...
        subagent_latest_input_at: '2026-06-11T00:00:00.000Z',
        subagent_latest_input_state_sequence: 7
      })
