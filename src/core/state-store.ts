...
function isValidPersistedSession(value: unknown): value is PersistedSession {
  return typeof value === 'object'
    ...
    && hasSessionTitleGenerationContext(value, 'title_generation')
...
function hasSessionTitleGenerationContext(value: object, key: string): boolean {
  if (!(key in value)) {
    return false
  }
  ...
  const context = field as Record<string, unknown>
  return ('prompt' in context) && (context.prompt === null || typeof context.prompt === 'string')
    && ('assistantSnippet' in context) && (context.assistantSnippet === null || typeof context.assistantSnippet === 'string')
    && ('contextUpdatedAt' in context) && (context.contextUpdatedAt === null || typeof context.contextUpdatedAt === 'string')
    && ('autoGeneratedTurnEpoch' in context) && (context.autoGeneratedTurnEpoch === null || typeof context.autoGeneratedTurnEpoch === 'number')
}
