...
describe('SessionRuntimeController', () => {
  ...
  describe('notifyUserInput', () => {
    test('transitions blocked session to working (optimistic unblock)', async () => {
      const { window: win, sent } = createMockWindow()
      const project = await manager.createProject({ path: await createTestWorkspace('ctrl-unblock-'), name: 'test' })
      const session = await manager.createSession({ projectId: project.id, type: 'claude-code', title: 'Claude' })
      await manager.markRuntimeAlive(session.id, 'ext-1')
      await manager.applySessionStatePatch(providerPatch(session.id, 'agent.permission_requested', 2, 'Permission needed', {
        blockingReason: 'permission'
      }))
      const observability = new ObservabilityService(new InMemoryObservationStore(), {
      ...
      const before = manager.snapshot().sessions[0]!
      expect(before.agentState).toBe('blocked')
      expect(before.blockingReason).toBe('permission')

      await controller.notifyUserInput(session.id)

      const after = manager.snapshot().sessions[0]!
      expect(after.agentState).toBe('working')
      expect(after.blockingReason).toBeNull()
      expect(sent.some(event => event.channel === IPC_CHANNELS.observabilitySessionPresenceChanged)).toBe(true)
      const presence = sent.find(event => event.channel === IPC_CHANNELS.observabilitySessionPresenceChanged)?.data as Record<string, unknown>
      expect(presence).toMatchObject({ sessionId: session.id, phase: 'running', agentState: 'working' })
    })

    test('is no-op for non-blocked sessions', async () => {
      const { window: win, sent } = createMockWindow()
      const project = await manager.createProject({ path: await createTestWorkspace('ctrl-noop-'), name: 'test' })
      const session = await manager.createSession({ projectId: project.id, type: 'shell', title: 'S1' })
      await manager.markRuntimeAlive(session.id, null)
      const controller = new SessionRuntimeController(manager, () => win)

      const beforeSequence = manager.snapshot().sessions[0]!.lastStateSequence
      await controller.notifyUserInput(session.id)

      const after = manager.snapshot().sessions[0]!
      expect(after.lastStateSequence).toBe(beforeSequence)
      expect(sent).toHaveLength(0)
    })

    test('is no-op for unknown session', async () => {
      const { window: win, sent } = createMockWindow()
      const controller = new SessionRuntimeController(manager, () => win)

      await controller.notifyUserInput('nonexistent-session')

      expect(sent).toHaveLength(0)
    })

    test('allocates monotonically increasing sequences', async () => {
      const { window: win } = createMockWindow()
      const project = await manager.createProject({ path: await createTestWorkspace('ctrl-seq-'), name: 'test' })
      const session = await manager.createSession({ projectId: project.id, type: 'claude-code', title: 'Claude' })
      await manager.markRuntimeAlive(session.id, 'ext-1')
      await manager.applySessionStatePatch(providerPatch(session.id, 'agent.permission_requested', 2, 'Permission needed', {
        blockingReason: 'permission'
      ...
      await controller.notifyUserInput(session.id)
      const seq1 = manager.snapshot().sessions[0]!.lastStateSequence

      await manager.applySessionStatePatch(providerPatch(session.id, 'agent.permission_requested', seq1 + 1, 'Blocked again', {
        blockingReason: 'permission'
      }))
      await controller.notifyUserInput(session.id)
      const seq2 = manager.snapshot().sessions[0]!.lastStateSequence

      expect(seq2).toBeGreaterThan(seq1)
    })

    test('coexists with provider hook events (self-correcting)', async () => {
      const { window: win } = createMockWindow()
      const project = await manager.createProject({ path: await createTestWorkspace('ctrl-coex-'), name: 'test' })
      const session = await manager.createSession({ projectId: project.id, type: 'claude-code', title: 'Claude' })
      await manager.markRuntimeAlive(session.id, 'ext-1')
      await manager.applySessionStatePatch(providerPatch(session.id, 'agent.permission_requested', 2, 'Permission needed', {
        blockingReason: 'permission'
      ...
      await controller.notifyUserInput(session.id)
      expect(manager.snapshot().sessions[0]!.agentState).toBe('working')

      const workingSeq = manager.snapshot().sessions[0]!.lastStateSequence
      await controller.applyProviderStatePatch(providerPatch(session.id, 'agent.turn_completed', workingSeq + 1, 'Turn done'))

      const final = manager.snapshot().sessions[0]!
      expect(final.agentState).toBe('idle')
      expect(final.hasUnseenCompletion).toBe(true)
    })
  })
})
