...
describe('WorkspaceHierarchyPanel', () => {
  ...
  describe('selection and archive events', () => {
    ...
    it('right-clicking a session can emit regenerateSessionTitle', async () => {
      const wrapper = mountPanel()
      await wrapper.findAll('.route-item.child')[0]!.trigger('contextmenu', {
        clientX: 144,
        clientY: 200
      ...
      const button = document.querySelector('[data-testid="session-context-menu.item.regenerate-title"]') as HTMLButtonElement | null
      expect(button).not.toBeNull()
      button!.click()

      expect(wrapper.emitted('regenerateSessionTitle')).toEqual([['session_1']])
    })
  })
  ...
  describe('style contracts', () => {
    it('keeps main-branch tokenized styling and avoids hardcoded visual regressions', () => {
      ...
      expect(source).toContain('.route-item--active::before')
      expect(source).toContain('.route-header-button')
      expect(source).toContain('background: var(--mica-alt);')
      expect(source).toContain('background: var(--active-fill);')
      expect(source).toContain('background: var(--control-fill-hover);')
      expect(source).toContain('background: var(--control-fill-active);')
      expect(source).toContain('border-color: var(--stroke-control);')
      ...
      expect(source).not.toContain('hover:bg-black-soft')
      expect(source).not.toContain('focus-visible:bg-black-soft')
      expect(source).not.toContain('border-line')
      expect(source).not.toContain('bg-surface-soft')
