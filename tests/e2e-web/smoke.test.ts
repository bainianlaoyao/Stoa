...
test.describe('Web smoke sentinel', () => {
  test('boot shell spec', async ({ page }) => {
    ...
    try {
      ...
      await expect(app.page.locator('[data-activity-item="sidebar-toggle"]')).toHaveCount(0)
    } finally {
      const { stateDir } = app
      await app.close()
  ...
  test('empty state spec', async ({ page }) => {
    const app = await launchWebApp(page)
  ...
  test('activity icons remain visible while switching surfaces', async ({ page }) => {
    const app = await launchWebApp(page)
