...
describe('playwright skeleton generator', () => {
  it('generates a deterministic session restore skeleton', () => {
    ...
    expect(generated).toContain('const projectRow = await createProject(app.page')
    expect(generated).toContain('await createSession(app.page, projectRow')
    expect(generated).toContain("app.page.getByRole('button', { name: `Archive ${sessionTitle}` }).click()")
