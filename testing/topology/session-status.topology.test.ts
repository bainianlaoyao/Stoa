...
describe('session status topology', () => {
  it('declares stable command route test ids for telemetry status assertions', () => {
    ...
    expect(sessionStatusTopology.testIds.statusDot).toBe('session-status-dot')
    expect(sessionStatusTopology.testIds.statusPhaseAttribute).toBe('data-session-status-testid')
